...
class Channel:
    ...
    def outp(self, command: str) -> str:
        ...
        while i < len(sub_cmds):
            ...
            if cmd == "LOAD":
                ...
                if param == "50":
                    # Derivative values
                    cvals["AMP"] = f"{float(cvals['AMP']) / 2:g}"
                    cvals["AMPVRMS"] = f"{float(cvals['AMPVRMS']) / 2:g}"
                    cvals["AMPDBM"] = f"{float(cvals['AMPDBM']) / 2:g}"
                    cvals["HLEV"] = f"{float(cvals['HLEV']) / 2:g}"
                    cvals["LLEV"] = f"{float(cvals['LLEV']) / 2:g}"
                elif param == "HZ":
                    # Derivative values
                    cvals["AMP"] = f"{float(cvals['AMP']) * 2:g}"
                    cvals["AMPVRMS"] = f"{float(cvals['AMPVRMS']) * 2:g}"
    ...
    def bswv(self, command) -> str:
        ...
        if cmd == "FRQ":
            ...
            amp = max(amp, 0.002)
            amp = min(amp, 20)
            cvals["AMP"] = float_to_str(amp)
            # Vrms = Vpp * 1/sqrt(2) / 2 = Vpp * .3535
            cvals["AMPVRMS"] = mul(a=cvals["AMP"], b=".3535")
            cvals["HLEV"] = div(n=cvals["AMP"], d="2")
            cvals["LLEV"] = sub(a=cvals["HLEV"], b=cvals["AMP"])
...
class SiglentDevice:
    ...
    others do not. For instance, not all support the CHDR command. Make this
    class adapt accordingly. Or, require a derived class for each model?
    """
    ...
    def storelist(self, command: str) -> str:
        """Process the command, update state, optionally return a result."""
        if command == "STL?":
    ...
    def parameter_copy(self, command: str) -> str:
        ...
        try:
            ...
            dest = channel_to_index(params[0])
            source = channel_to_index(params[1])
        except Exception as err:
            errlog.exception(err)
            return ""
